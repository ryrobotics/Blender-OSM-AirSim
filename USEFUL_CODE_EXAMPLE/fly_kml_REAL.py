"""
Read KML format flight path EXPORTED BY DJI REAL FLIGHT and Fly in the 2-D space
"""
import re
import time
...
def main():
    ...
    # client.simSetTraceLine([1.0, 0.0, 0.0, 0.5], 50) # RGB Alpha
    ...
    print("Going Higher")
    client.moveToPositionAsyncGeo(gps=gps_new, velocity=2).join()
    ...
    for key, gps_xml in xml.items():
        ...
        gps_new = (gps_xml[0], gps_xml[1], float(gps_xml[2])-138)
        print("Moving to: " + gps_xml[0] + ', ' + gps_xml[1] + ', ' + gps_xml[2])
        client.simSetTraceLine([1.0, 0.0, 0.0, 0.5], 10) # RGB Alpha
        client.moveToPositionAsyncGeo(gps=gps_new, velocity=2,).join()
        client.simSetTraceLine([0.0, 1.0, 0.0, 0.5], 20) # RGB Alpha
        time.sleep(0.01)
        if key is 1:
            print('Ready to ACT!')
            client.simSetTraceLine([1.0, 0.0, 0.0, 0.5], 10) # RGB Alpha
